...
export const useStore = defineStore("app", {
    state: () => {
        return {
            ...
            isCurrentlyRunningLesson: false,

            currentLessonSteps: [] as LessonStepDetails[],

            currentLessonObject: null as Lesson | null,
    ...
    getters: { 
        ...
        // Returns as Lesson[]
        getAllLoadedLessons: (state) => {
            return state.loadedLessons;
        },

        getLoadedLessonsLength: (state) => {
            return state.loadedLessons.length;
        },

        getCurrentLesson: (state) => {
            return state.currentLessonObject;
        },
    ...
    actions:{
        ...
        setLessonInTestModeStatus(isTestMode: boolean) {
        ...
        // Modifying the loaded lessons.

        clearLoadedLessonFiles() {
            this.loadedLessons.length = 0; // Deletes ALL uploaded lessons
        },

        deleteLoadedLesson(i: number) {
            this.loadedLessons.splice(i, 1); // Removes 1 element at index i
        },

        newLoadedLesson(lesson: Lesson) {
            this.loadedLessons.push(lesson);
        },
